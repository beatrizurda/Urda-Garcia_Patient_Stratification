...
def pairwise_union_cosine_distance(dis1,dis2,genes='sDEGs'):
   ''' ''' 
#   dis1 = 'Asthma'
##   dis2 = 'Asthma'
#   dis2 = 'Schizophrenia'
...
#   genes='DM'
   ...
   if ((genes == 'sDEGs') or (genes == 'allgenes')):
       ...
       if(len(expr1['symbol']) > 1): 
           val = float(cosine_similarity(np.array([expr1['logFC']]), np.array([expr2['logFC']])))
#           val,pv = spearmanr(list(expr1['logFC']), list(expr2['logFC']))
           
           # Compute the significance of the similarity
           cngenes = expr1.shape[0]
           rcosine = list()
           niter = 10000
           for k in range(niter):
               ran1 = random.sample(list(expr1['logFC']), cngenes)
               ccosine = float(cosine_similarity(np.array([ran1]), np.array([expr2['logFC']])))
               rcosine.append(ccosine)
           if val >= 0:
               pv = sum(map(lambda x: x>val, rcosine)) / niter   # Map the function of the list to get the number --> get a T/F list, we will sum the Trues to get the number of elements that meet the condition. Then, we divide it bu the total n of randomizations. 
           else:
               pv = sum(map(lambda x: x<val, rcosine)) / niter
               
           # To revise how this was done: https://thispointer.com/python-count-elements-in-a-list-that-satisfy-certain-conditions/
...
# REV1
sDEGs_matrix, pvmatrix, outdf = build_distance_matrix(dis_list, pairwise_union_cosine_distance) 
