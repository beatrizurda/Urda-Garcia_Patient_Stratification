...
output_path = "Other_molecular_networks/"
network_path = "Other_molecular_networks/Transformed_networks/"
universe_option <- "B"
randomizing_labels <-  TRUE

# MICROBIOME
filename = "icd9_microbiome_network.txt"; filename = "my_icd9_microbiome_network.txt"
# MICRO-RNAs
filename = "icd9_microRNAs_network.txt"; filename = "my_icd9_microRNAs_network.txt"
# PPIs
filename = "icd9_ppi_network.txt"; filename = "my_icd9_ppi_network.txt"

compute_Barabasi_overlap_icd9_level <- function(network_path, filename, output_path, universe_option, randomizing_labels){
  # SCRIPT BEGINS
  filedir <- 'Network_building/Defined_networks/'
  filepath <- paste(filedir,filename,sep="")
  if(randomizing_labels == TRUE){
    outpath <- paste(output_path,'Overlapping_results/Shuffling_labels/',gsub('_network.txt',"",filename),'_',universe_option,'_',randomizing_labels,'_overlap.txt',sep="")
  }else{
    outpath <- paste(output_path,'Overlapping_results/Sampling_randomly/',gsub('_network.txt',"",filename),'_',universe_option,'_',randomizing_labels,'_overlap.txt',sep="")
  }
  # outpath <- paste('Network_building/Overlapping_results/',gsub('_network.txt',"",filename),'_',universe_option,'_overlap.txt',sep="")
  network <- read.csv(paste(network_path,filename,sep=""),header=T, sep="\t",stringsAsFactors = F)
  ...
  }else if (length(grep("sDVGs",filename)) == 1){     # If the network is based on DExpression
  ...
  # Remove duplicated interactions --> This step is not needed for our networks or barabasi's network since they are not directed.
  remove_duplicated_interactions <- function(df){
    df$dup <- paste(df$Dis1, df$Dis2)
    if(length(unique(df$dup)) == length(df$dup)){
      print("No duplicated interactions") 
    }else{
      # print(length(df$dup))
      print(length(unique(df$dup)))
  ...
  dim(cnetwork)
  cnetwork <- remove_duplicated_interactions(cnetwork); dim(cnetwork)
  
  # Compute number of interactions
  length(cnetwork$Dis1) # 167
  My_interactions <- length(cnetwork$Dis1)
  write(paste('Unique icd9 codes in the network:\t',length(unique(union(cnetwork$Dis1, cnetwork$Dis2))),sep=""),file=outpath, append=TRUE)
  write(paste('Unique icd9 interactions:\t',My_interactions,sep=""),file=outpath, append=TRUE)
  write.table(cnetwork, file= paste("Other_molecular_networks/Transformed_networks/","Nodups_",filename,sep=""), sep="\t",row.names=F, quote = FALSE)
  ...
  write(paste('Interaction of the network within common icd9 codes:\t',dim(cnetwork)[1],sep=""),file=outpath, append=TRUE)
  ...
  if(randomizing_labels == TRUE){
    hist_outpath <- paste(output_path,'Overlapping_results/Shuffling_labels/histograms/',gsub('_network.txt',"",filename),'_',universe_option,'_',randomizing_labels,'_hist.pdf',sep="")
  }else{
    hist_outpath <- paste(output_path,'Overlapping_results/Sampling_randomly/histograms/',gsub('_network.txt',"",filename),'_',universe_option,'_',randomizing_labels,'_hist.pdf',sep="")
  }
...
compute_Barabasi_overlap_icd9_level(network_path, filename, output_path, universe_option, randomizing_labels)
